...
int closestJet(const TLorentzVector& lep_p4, std::vector<TLorentzVector>& v_Jets, float dRmin, float maxAbsEta, TLorentzVector& jet_p4)
{
  ...
  for (unsigned int pfjidx=0; pfjidx<v_Jets.size(); ++pfjidx)
  {
    ...
    {
      float tmp_dRmin = jet_p4.DeltaR(lep_p4);
      if (tmp_dRmin < dRmin)
      {
        returnValue = pfjidx;
	dRmin = tmp_dRmin;
...
int ReadUCSDBabyTuples(std::string infile, std::string treeStr, std::string Sample, std::string trig, std::string Trigger="None")
{
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    //if(v_selectedJets.size() > 0) std::cout << "v_selectedJets.at(0).Pt() = " << v_selectedJets.at(0).Pt() << std::endl;
    //OS
    if((lep_pdgId->at(0)*lep_pdgId->at(1)==-169) and (nLlep==2))//mm
    //if((lep_pdgId->at(0)*lep_pdgId->at(1)==-169) and (nVlep==2)*(nLlep==2)*(nTlep==2))//mm
    {
      ...
      int index0 = closestJet(mu1, v_selectedJets, 7.0, 3.0, jet0_p4);
      int index1 = closestJet(mu2, v_selectedJets, 7.0, 3.0, jet1_p4);
      if(index0>-1 and index1>-1 and index0!=index1 and jet0_p4.Pt() > 30.0 and jet1_p4.Pt() > 30.0)
      {
        //At this point, jet0 and jet1 should contain the closest jets to mu1 and mu2, respectively.
        float ptratio_leading = (jet0_p4.Pt() > 0. ? mu1.Pt()/jet0_p4.Pt() : 1.);
        float ptratio_trailing = (jet1_p4.Pt() > 0. ? mu2.Pt()/jet1_p4.Pt() : 1.);
        float spatialmu1_2 = mu1.X()*mu1.X() + mu1.Y()*mu1.Y() + mu1.Z()*mu1.Z();
        float spatialmu2_2 = mu2.X()*mu2.X() + mu2.Y()*mu2.Y() + mu2.Z()*mu2.Z(); 
        float mu1Xjet1 = mu1.X()*jet0_p4.X() + mu1.Y()*jet0_p4.Y() + mu1.Z()*jet0_p4.Z();
        float mu2Xjet2 = mu2.X()*jet1_p4.X() + mu2.Y()*jet1_p4.Y() + mu2.Z()*jet1_p4.Z();
        /*std::cout << "mu1Xjet1 = " << mu1Xjet1 << std::endl;
        std::cout << "mu2Xjet2 = " << mu2Xjet2 << std::endl;
        std::cout << "(jet0_p4.X()*jet0_p4.X() + jet0_p4.Y()*jet0_p4.Y() + jet0_p4.Z()*jet0_p4.Z()) = " << (jet0_p4.X()*jet0_p4.X() + jet0_p4.Y()*jet0_p4.Y() + jet0_p4.Z()*jet0_p4.Z()) << std::endl;
        std::cout << "(jet1_p4.X()*jet1_p4.X() + jet1_p4.Y()*jet1_p4.Y() + jet1_p4.Z()*jet1_p4.Z()) = " << (jet1_p4.X()*jet1_p4.X() + jet1_p4.Y()*jet1_p4.Y() + jet1_p4.Z()*jet1_p4.Z()) << std::endl;
        std::cout << "jet0_p4.X() = " << jet0_p4.X() << std::endl;
        std::cout << "jet1_p4.X() = " << jet1_p4.X() << std::endl;
        std::cout << "jet0_p4.Pt() = " << jet0_p4.Pt() << std::endl;
        std::cout << "jet1_p4.Pt() = " << jet1_p4.Pt() << std::endl;*/
        float pLrel1_2 = mu1Xjet1*mu1Xjet1/(jet0_p4.X()*jet0_p4.X() + jet0_p4.Y()*jet0_p4.Y() + jet0_p4.Z()*jet0_p4.Z());
        float pLrel2_2 = mu2Xjet2*mu2Xjet2/(jet1_p4.X()*jet1_p4.X() + jet1_p4.Y()*jet1_p4.Y() + jet1_p4.Z()*jet1_p4.Z());
        float pTrel1_2 = spatialmu1_2 - pLrel1_2;
        float pTrel2_2 = spatialmu2_2 - pLrel2_2;
        float ptrel0 = sqrt(pTrel1_2);
        float ptrel1 = sqrt(pTrel2_2);
        if(Sample=="MC" and mc_HLT_DoubleMu==1)
        {
          fillMuHistCollection(mumuHistCut1, mu1.Pt(), mu2.Pt(), mu1.Eta(), mu2.Eta(), mu1.Phi(), mu2.Phi(), ptratio_leading, ptratio_trailing, fabs(lep_ip3d->at(0)), fabs(lep_ip3d->at(1)), fabs(lep_dxy->at(0)), fabs(lep_dxy->at(1)), fabs(lep_dz->at(0)), fabs(lep_dz->at(1)), ptrel0, ptrel1, lep_relIso04EAv2->at(0), lep_relIso04EAv2->at(1), jet0_p4.DeltaR(mu1), jet1_p4.DeltaR(mu2), (mu1+mu2).M(), met_pt, nj30, nb, jet1pt, jet2pt, jet1eta, jet2eta, jet1phi, jet2phi, bjet1csv, bjet2csv, bjet1pt, bjet1eta, bjet1phi, bjet2pt, bjet2eta, bjet2phi, weight);
    ...
    //else if((lep_pdgId->at(0)*lep_pdgId->at(1)==-143) and (nLlep==2))//em
    ...
    else if((lep_pdgId->at(0)*lep_pdgId->at(1)==-121) and (nLlep==2))//ee
