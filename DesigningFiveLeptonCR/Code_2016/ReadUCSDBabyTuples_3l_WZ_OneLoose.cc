#include "ReadUCSDBabyTuples_2016.h"
...
int ReadUCSDBabyTuples_3l_WZ_OneLoose(std::string infile, std::string treeStr, std::string Sample, std::string Trigger="None")
{
  ...
  TH2F  *sfElRecoLowPtHist = (TH2F*) sfElRecoLowPtFile->Get("EGamma_SF2D");
  ...
  TFile *sfMuRecoLowPtFile = new TFile("EfficiencyStudies_2016_rootfiles_lowpt_RunBCDEF_SF_ID.root");
  TH2F  *sfMuRecoLowPtHist = (TH2F*) sfMuRecoLowPtFile->Get("NUM_MediumID_DEN_genTracks_pt_abseta");
  TFile *sfMuRecoLowPtFile2 = new TFile("EfficiencyStudies_2016_rootfiles_lowpt_RunGH_SF_ID.root");
  TH2F  *sfMuRecoLowPtHist2 = (TH2F*) sfMuRecoLowPtFile2->Get("NUM_MediumID_DEN_genTracks_pt_abseta");
  TFile *sfMuIsoFile = new TFile("EfficiencyStudies_2016_rootfiles_RunBCDEF_SF_ISO.root");
  TH2F  *sfMuIsoHist = (TH2F*) sfMuIsoFile->Get("NUM_LooseRelIso_DEN_MediumID_eta_pt");
  TFile *sfMuIsoFile2 = new TFile("EfficiencyStudies_2016_rootfiles_RunGH_SF_ISO.root");
  TH2F  *sfMuIsoHist2 = (TH2F*) sfMuIsoFile2->Get("NUM_LooseRelIso_DEN_MediumID_eta_pt");
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    for (unsigned int ielectron=0; ielectron<lep_p4->size(); ielectron++)
    {
       ...
       electron.sf = recoSFEl(sfElRecoFile, idSFElRecoHist, lep_pt->at(ielectron), lep_eta->at(ielectron))*recoLowPtSFEl(sfElRecoLowPtFile, sfElRecoLowPtHist, lep_pt->at(ielectron), lep_eta->at(ielectron))*vetoSFEl(sfElVetoFile, idSFElVetoHist, lep_pt->at(ielectron), lep_eta->at(ielectron));
