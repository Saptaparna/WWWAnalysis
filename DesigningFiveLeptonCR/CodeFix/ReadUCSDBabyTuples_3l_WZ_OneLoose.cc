...
int ReadUCSDBabyTuples_3l_WZ_OneLoose(std::string infile, std::string treeStr, std::string Sample, std::string Trigger="None")
{
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    for (unsigned int imuon=0; imuon<lep_p4->size(); imuon++)
    {
       ...
       //if(abs(lep_id->at(imuon))==13 and muon.pt>10.0 and fabs(lep_eta->at(imuon)) < 2.4 and fabs(lep_relIso04DB->at(imuon)) < 0.25 and fabs(lep_sip3d->at(imuon)) < 4 and lep_isMediumPOG->at(imuon)==1)
       if(abs(lep_id->at(imuon))==13 and muon.pt>10.0 and fabs(lep_eta->at(imuon)) < 2.4 and fabs(lep_sip3d->at(imuon)) < 4 and lep_isMediumPOG->at(imuon)==1 and fabs(lep_relIso04DB->at(imuon)) < 0.25) 
       {
    ...
    if(v_muons.size()==3 and v_electrons.size()==1 and (elecIsoCut(v_electrons.at(0).eta, v_electrons.at(0).pt, v_electrons.at(0).iso)==false))
