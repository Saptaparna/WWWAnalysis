#include "ReadUCSDBabyTuples_2017.h"
...
int ReadUCSDBabyTuples_3l_WZ_OneLoose(std::string infile, std::string treeStr, std::string Sample, std::string Trigger="None")
{
  ...
  TFile *sfElRecoFile = new TFile("egammaEffi.txt_EGM2D_runBCDEF_passingRECO.root");
  TH2F  *idSFElRecoHist = (TH2F*) sfElRecoFile->Get("EGamma_SF2D"); //eta,pt
  TFile *sfElRecoFileLowPt = new TFile("egammaEffi.txt_EGM2D_runBCDEF_passingRECO_lowEt.root");
  TH2F  *idSFElRecoHistLowPt = (TH2F*) sfElRecoFileLowPt->Get("EGamma_SF2D"); //eta,pt
  TFile *sfElVetoFile = new TFile("2017_ElectronWPVeto_Fall17V2.root"); 
  TH2F  *idSFElVetoHist = (TH2F*) sfElVetoFile->Get("EGamma_SF2D");//eta, pt

  TFile *sfMuRecoFile = new TFile("EfficiencyStudies_2017_rootfiles_RunBCDEF_SF_ID.root");
  TH2F  *sfMuRecoHist = (TH2F*) sfMuRecoFile->Get("NUM_MediumID_DEN_genTracks_pt_abseta");// x=pt, y=abseta
  TFile *sfMuRecoLowPtFile = new TFile("EfficiencyStudies_2017_rootfiles_lowpt_RunBCDEF_SF_ID_JPsi.root");
  TH2F  *sfMuRecoLowPtHist = (TH2F*) sfMuRecoLowPtFile->Get("NUM_MediumID_DEN_genTracks_pt_abseta");//x=pt, y=abseta
  TFile *sfMuIsoFile = new TFile("EfficiencyStudies_2017_rootfiles_RunBCDEF_SF_ISO.root");
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    double weight = evt_scale1fb*getTruePUw2017(nTrueInt);
    ...
    for (unsigned int ielectron=0; ielectron<lep_p4->size(); ielectron++)
    {
       ...
       electron.sf = recoSFEl(sfElRecoFile, idSFElRecoHist, lep_pt->at(ielectron), lep_eta->at(ielectron))*recoLowPtSFEl(sfElRecoFileLowPt, idSFElRecoHistLowPt, lep_pt->at(ielectron), lep_eta->at(ielectron))*vetoSFEl(sfElVetoFile, idSFElVetoHist, lep_pt->at(ielectron), lep_eta->at(ielectron));
     ...
     if(v_muons.size()==3 and v_electrons.size()==1 and v_muons.at(0).iso < 0.25 and v_muons.at(1).iso < 0.25 and v_muons.at(2).iso < 0.25 and (elecIsoCut(v_electrons.at(0).eta, v_electrons.at(0).pt, v_electrons.at(0).iso)==false))
    {
      ...
      if(nb==0)
      {
        ...
        //if(fabs((v_muons.at(0).lep_lv+v_muons.at(1).lep_lv).M() - 91.1875) < 10 or fabs((v_muons.at(1).lep_lv+v_muons.at(2).lep_lv).M() - 91.1875) < 10 or fabs((v_muons.at(0).lep_lv+v_muons.at(2).lep_lv).M() - 91.1875) < 10)
        {
          ...
          if(v_electrons.at(0).lep_lv.Pt() > 17.0 and v_muons.at(0).lep_lv.Pt() > 10.0 and v_muons.at(1).lep_lv.Pt() > 10.0)
