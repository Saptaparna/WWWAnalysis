...
Bool_t FatJetAnalyzer::Process(Long64_t entry)
{
    ...
    for (int i=0; i < fMuonArr->GetEntries(); i++) 
    {
      ...
      if (muonP4.Pt() > 10) 
      {
        ...
        /*if(isTightMuon(muon)==1) 
        {
          std::cout << "muon->pt = " << muon->pt << std::endl;
          std::cout << "muon->eta = " << muon->eta << std::endl; 
          std::cout << "muon->phi = " << muon->phi << std::endl;
          std::cout << "muon->nPixHits = " << muon->nPixHits << std::endl;
          std::cout << "muon->d0 = " << muon->d0 << std::endl;
          std::cout << "muon->nTkLayers = " << muon->nTkLayers << std::endl;
          std::cout << "muon->nValidHits = " << muon->nValidHits << std::endl;
        }*/
        bool isTight = false;
        if(muon->pt > 5.0 and 
        fabs(muon->eta) < 2.4 and 
        (muon->typeBits & baconhep::kGlobal) and 
        muon->muNchi2 < 10. and 
        muon->nMatchStn  > 1 and 
        muon->nPixHits > 0 and 
        fabs(muon->d0) < 0.2 and 
        fabs(muon->dz) < 0.5 and 
        muon->nTkLayers > 5 and 
        muon->nValidHits > 0) isTight = true;
        else isTight = false;
...
Bool_t FatJetAnalyzer::isTightMuon(baconhep::TMuon* muon)
{
  ...
  if (fabs(muon->eta) < 2.4 and muon->typeBits & baconhep::kGlobal) 
  {
    ...
    //if(muon->nMatchStn <= 1)   return false;
    //if(muon->nPixHits  <= 0)   return false;
    if(fabs(muon->d0)  >= 0.005) return false;
    if(fabs(muon->dz)  >= 0.1)   return false;
    if(muon->sip3d     > 4.0)    return false;
    //if(muon->nTkLayers <= 5)   return false;
    //if(muon->nValidHits <= 0)  return false;
  }
  return true;
