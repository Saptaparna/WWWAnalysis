...
int ReadUCSDBabyTuples(std::string infile, std::string treeStr, std::string Sample, std::string Trigger="None")
{
  ...
  for (int i=0; i<nEvents; ++i)
  {
    ...
    std::vector<leptonInfo> zcand_muons;
    zcand_muons.clear();
    ...
    for (unsigned int imuon=0; imuon<lep_p4->size(); imuon++)
    {
       ...
       if(abs(lep_id->at(imuon))==13 and muon.pt>0.0 and (fabs(lep_eta->at(imuon)) < 2.4) and (fabs(lep_dz->at(imuon)) < 0.1) and (fabs(lep_dxy->at(imuon)) < 0.05) and fabs(lep_relIso03EA->at(imuon) < 0.40) and (fabs(lep_sip3d->at(imuon)) < 5)) 
       {
         v_muons.push_back(muon);
         v_leptons.push_back(muon);
       } 
       if(abs(lep_id->at(imuon))==13 and lep_pt->at(imuon)>0.0 and (fabs(lep_eta->at(imuon)) < 2.4) and (fabs(lep_dz->at(imuon)) < 0.1) and (fabs(lep_dxy->at(imuon)) < 0.05) and (fabs(lep_relIso03EA->at(imuon)) < 0.20) and (fabs(lep_sip3d->at(imuon)) < 5)) zcand_muons.push_back(muon);
    }
    ...
    std::vector<leptonInfo> zcand_electrons;
    zcand_electrons.clear();   
    ...
    for (unsigned int ielectron=0; ielectron<lep_p4->size(); ielectron++)
    {
       ...
       if(abs(lep_id->at(ielectron))==11 and electron.pt>0.0 and (fabs(lep_eta->at(ielectron)) < 2.4) and (fabs(lep_dz->at(ielectron)) < 0.1) and (fabs(lep_dxy->at(ielectron)) < 0.05) and (fabs(lep_relIso03EA->at(ielectron)) < 0.4) and (fabs(lep_sip3d->at(ielectron)) < 5)) 
       {
         v_electrons.push_back(electron);
         v_leptons.push_back(electron);
       }
       if(abs(lep_id->at(ielectron))==11 and electron.pt>0.0 and (fabs(lep_eta->at(ielectron)) < 2.4) and (fabs(lep_dz->at(ielectron)) < 0.1) and (fabs(lep_dxy->at(ielectron)) < 0.05) and (fabs(lep_relIso03EA->at(ielectron)) < 0.2) and (fabs(lep_sip3d->at(ielectron)) < 5)) zcand_electrons.push_back(electron);
    }
    ...
    int nzcand_leptons = zcand_muons.size() + zcand_electrons.size(); 
    if(v_muons.size()==4)
    {
      ...
      if(minChi2 < 100.0)
      {
        if(Sample=="MC") 
        {
          ...
          h_TotalEvents_4Mu->Fill(2, weight); 
          if(met_pt > 40.0)
          { 
            h_TotalEvents_4Mu->Fill(3, weight);
            if(nb==0) h_TotalEvents_4Mu->Fill(4, weight);
          }
        }
        else if(Sample=="Data") 
        {
          h_min_chi_weight->Fill(minChi2, 1.0);
          fillMuHistCollection4l(Cut1_4l, v_muons.at(0).pt, v_muons.at(1).pt, v_muons.at(2).pt, v_muons.at(3).pt, v_muons.at(0).eta, v_muons.at(1).eta, v_muons.at(2).eta, v_muons.at(3).eta, v_muons.at(0).phi, v_muons.at(1).phi, v_muons.at(2).phi, v_muons.at(3).phi, v_muons.at(0).sip3d, v_muons.at(1).sip3d, v_muons.at(2).sip3d, v_muons.at(3).sip3d, v_muons.at(0).dxy, v_muons.at(1).dxy, v_muons.at(2).dxy, v_muons.at(3).dxy, v_muons.at(0).dz, v_muons.at(1).dz, v_muons.at(2).dz, v_muons.at(3).dz, met_pt, nj, nb, (mu1+mu2+mu3+mu4).M(), (v_muons.at(lep_zcand_idx1).lep_lv + v_muons.at(lep_zcand_idx2).lep_lv).M(), (mu2+mu3).M(), (mu3+mu4).M(),  (mu4+mu1).M(), 1.0);
          h_TotalEvents_4Mu->Fill(2, 1.0);
          if(met_pt > 40.0)
          { 
            h_TotalEvents_4Mu->Fill(3, 1.0);
            if(nb==0) h_TotalEvents_4Mu->Fill(4, 1.0);
          }
        }
      }
    }
    else if(v_electrons.size()==4)
    {
      ...
      if(minChi2 < 100.0)
      {
        if(Sample=="MC") 
        {
          ...
          h_TotalEvents_4El->Fill(2, weight);
          if(met_pt > 40.0) 
          {
            h_TotalEvents_4El->Fill(3, weight);
            if(nb==0) h_TotalEvents_4El->Fill(4, weight);
          } 
        }
        else if(Sample=="Data") 
        {
          ...
          h_TotalEvents_4El->Fill(2, 1.0);
          if(met_pt > 40.0)
          { 
            h_TotalEvents_4El->Fill(3, 1.0);
            if(nb==0) h_TotalEvents_4El->Fill(4, 1.0);
          }
        }
      }
    }
    if(v_muons.size()==2 and v_electrons.size()==2)
    {
      ...
      if(Sample=="MC") 
      {
        ...
        h_TotalEvents_2El2Mu->Fill(2, weight);
        if(met_pt > 40.0)
        { 
          h_TotalEvents_2El2Mu->Fill(3, weight);
          if(nb==0) h_TotalEvents_2El2Mu->Fill(4, weight);
        }
      } 
      else if(Sample=="Data") 
      {
        fillElMuHistCollection4l(Cut1_4lElMu, v_electrons.at(0).pt, v_electrons.at(1).pt, v_muons.at(0).pt, v_muons.at(1).pt, v_electrons.at(0).eta, v_electrons.at(1).eta, v_muons.at(0).eta, v_muons.at(1).eta, v_electrons.at(0).phi, v_electrons.at(1).phi, v_muons.at(0).phi, v_muons.at(1).phi, v_electrons.at(0).sip3d, v_electrons.at(1).sip3d, v_muons.at(0).sip3d, v_muons.at(1).sip3d, v_electrons.at(0).dxy, v_electrons.at(1).dxy, v_muons.at(0).dxy, v_muons.at(1).dxy, v_electrons.at(0).dz, v_electrons.at(1).dz, v_muons.at(0).dz, v_muons.at(1).dz, met_pt, nj, nb, (el1+el2+mu1+mu2).M(), (el1+el2).M(), (mu1+mu2).M(), 1.0);
        h_TotalEvents_2El2Mu->Fill(2, 1.0);
        if(met_pt > 40.0)
        { 
          h_TotalEvents_2El2Mu->Fill(3, 1.0);
          if(nb==0) h_TotalEvents_2El2Mu->Fill(4, 1.0);
        }
      }
    ...
    if(v_leptons.size()==5)
    {
      /*int lep_zcand_idx1, lep_zcand_idx2;
      lep_zcand_idx1=lep_zcand_idx1=0;
      bool ifpass;
      std::cout << "here" << std::endl;
      for (unsigned int jj = 0 ; jj < (v_leptons.size() - 1) ; jj ++)
      {
        for (unsigned int kk = 0 ; kk < v_leptons.size() ; kk ++)
        {
          TLorentzVector zcand = v_leptons.at(jj).lep_lv + v_leptons.at(kk).lep_lv;
          ifpass = true; //based on Philip's classification scheme
          //if (fabs(zcand.M() - 91.1875) > 15)  ifpass = false; // within Z mass window
          if (abs(v_leptons.at(jj).id) != abs(v_leptons.at(kk).id)) ifpass = false; // same-flavor
          if (v_leptons.at(jj).id == v_leptons.at(kk).id) ifpass = false; // opposite-sign
          //if (ifpass && fabs(zcand.M() - 91.1875) > compare) ifpass = false;
          if (ifpass)
          {
      ...
      double Mz = 91.1876;
      double chi1_sq_1el4mu,chi2_sq_1el4mu,chi1_sq_4el1mu,chi2_sq_4el1mu;
      ...
      bool Ztag = false;
      if((counter_el==2 and counter_mu==3) or (counter_el==3 and counter_mu==2))
      {
        if(counter_el==2 and counter_mu==3)
        {
          if(v_electrons.at(0).id*v_electrons.at(1).id==-121) Ztag=true;
          if(v_muons.at(0).id*v_muons.at(1).id==-149 or v_muons.at(1).id*v_muons.at(2).id==-149 or v_muons.at(0).id*v_muons.at(2).id==-149) Ztag=true;
        }
        if(counter_el==3 and counter_mu==2)
        {
          if(v_muons.at(0).id*v_muons.at(1).id==149) Ztag=true;
    ...
    if((v_muons.size()==5) or (v_electrons.size()==5) or (v_muons.size()==4 and v_electrons.size()==1) or (v_muons.size()==3 and v_electrons.size()==2) or (v_muons.size()==2 and v_electrons.size()==3) or (v_muons.size()==1 and v_electrons.size()==4)) nevent+=weight; 
