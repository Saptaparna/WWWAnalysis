...
int ReadPPZFiles_Delphes(std::string infile, std::string outfile){
  ...
  TH1D *h_WJ_PT = new TH1D("h_WJ_PT", "W_{J} pT; W_{J} pT [GeV]; Events", 1000, 0, 1000); h_WJ_PT->Sumw2();
  ...
  for (int i=0; i<nEvents ; ++i)
  {
   ...
   if((Jet1+Jet2).Pt() > 50.0) h_WJ_PT->Fill((Jet1+Jet2).Pt());
   //if((Jet1+Jet2).Pt() > 50.0 and (Jet1+Jet2).M() > 70 and (Jet1+Jet2).M() < 100) h_WJ_PT->Fill((Jet1+Jet2).Pt());
  }//event loop closed
